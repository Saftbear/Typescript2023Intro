...
import Misc from './createAdditional';
...
describe('Misc', () => {
    ...
    let testedClass: Misc;

    const path = 'path';
    const filename = 'filename.mp4';
    const filenameNoDot = 'test-filename';
    ...
    describe("create_preview", () => {
        it('should generate previews successfully and create necessary folders', async () => {
            ...
            // fs.mkdirSync should have been called with the correct arguments
            expect(mkdirSpy).toHaveBeenCalledWith(save_path, { recursive: true });
    ...
    describe('createShortVideo', () => {
        ...
        it('should merge short video clips successfully', async () => {
           ...
           // verify(ffmpegWrapperMock.mergeShortVideoClips(clips, outputPath, tempPath)).once(); 
           // could be an issue with array
