...
class Misc implements IAdditional{
    private path: string;
    ...
    public async create_thumbnail(path?: string, filename?: string): Promise<void> {
        path = path ?? this.path;
    ...
    public async createShortVideo(path?: string, filename?: string): Promise<void> {
        ...
        const save_path:string = 'uploaded_files/ShortVideos/';
        ...
        const interval: number = this.duration / num_clips;     
  
        let clips: Array<string> = new Array(); 
    
        // Generate clips
        for(let i = 0; i < num_clips; i++) {
            let start_time:number = i * interval;
            const output_filename:string = `${clipPath}/clip_${i}.mp4`;
            try {
                await this.ffmpegWrapper.createShortVideoClip(path, Math.floor(start_time), output_filename);
        ...
        const outputPath:string = `${clipPath}/output.mp4`;
